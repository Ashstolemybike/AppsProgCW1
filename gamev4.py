...
import sys
import pickle
##Reminder for the user to scroll up
while True:
    print ("------------------------------------------------------------------------")
...
while True:
    ...
    if len(name) > 10:
        ...
        break
        print ("------------------------------------------------------------------------")
##The starting text giving basic info
print ("------------------------------------------------------------------------")
...
print 
print ("Your escapade begins in Varrock, a small village located in the Grey \
Forest. The villagers called for you and asked for your help in dealing with the \
predator that has been killing the inhabitants of the forest. The Varrock Baron has \
...
APuzzle = Counter("Puzzle A Counter", "Puzzle A", 0) #The "counter" descriptions will never be read in game.

BPuzzle = Counter("Puzzle A Counter", "Puzzle B", 0) #The "counter" descriptions will never be read in game.

CPuzzle = Counter("Puzzle A Counter", "Puzzle C", 0) #The "counter" descriptions will never be read in game.
#-------------------------------------------------------------------------------
#Random items users can pick up during the game
class Misc(Item):
    def __init__(self, name, description):
...
#-------------------------------------------------------------------------------       
#2 quests the user can play through. WW quest must be completed first before the phoenix quest can be attempted
#The quests and their explanations
class Quest(Item):
    def __init__(self, name, description):
...
immense unrest among the population of Varrock. First, they started killing anyone \
who dared exit the safety of the village. The next thing you know, they raided the village, \
killing off half of its population. You have been hired by the Varrock Baron to \
...
killed, you need better armour. Rumors floating around state that a set of ogre \
armour lies in Lake Azure...")

PhoenixQ = Quest("Conflagration Termination", "The monsters of Jad Mountain have \
relentlessly attempted destroying all of Jad Town. The only thing standing in \
their way is the Jad Chief's Army. However, it will not last, it will perish if \
something is not done immediately. You will go into Jad Mountain, north of Jad Town, \
and kill the beasts that populate it. There must be a leader commanding the monstrous \
forces. More likely than not, it will be at the summit of Jad Mountain.")

PhoenixQ_Updated = Quest("Conflagration Termination", "The monsters of Jad Mountain \
have relentlessly attempted destroying all of Jad Town. The only thing standing \
in their way is the Jad Chief's Army. However, it will not last, it will perish \
if something is not done immediately. You will go into Jad Mountain, north of Jad \
Town, and kill the beasts that populate it. There must be a leader commanding the \
monstrous forces. More likely than not, it will be at the summit of Jad Mountain. \
UPDATE: Go to the Chief's Palace to receive your reward.")
#-------------------------------------------------------------------------------
#Class for the weapons the user is going to have
class Weapon(Item):
    def __init__(self, name, description):
        super(Weapon, self).__init__(name, description)
#-------------------------------------------------------------------------------        
#This is for sword that deal physical damage to enemies
...
Rune_Sword = Physical("Rune Sword", "Deadly and agile. Crafted \
by the blacksmiths of Varrock for you as a reward for the Werewolf Quest.", 10)

Dragon_Scimitar = Physical("Dragon Scimitar", "Great at killing things.", 20)

Obsidian_Sword = Physical("Obsidian Sword", "An ancient weapon used by the Knights \
of Jad to repel the beasts of The Mountain.", 20)
...
#-------------------------------------------------------------------------------
#This is for magical weapon that lifesteals
...
draining magic by the witches of Jad Mountain", 20)
#-------------------------------------------------------------------------------
#The armour the user is wearing
class Armour(Item):
...
Wolf_Mask = Headgear("Legendary Werewolf Mask", "A mask carved from the face \
...
Rune_Chest = Chest("Rune Chestplate", "Crafted by the ogres of the forest. \
Only the finest and strongest ores was used in the making of this chestplate.", 10)

Dragon_Chest = Chest("Dragon's Chest", "This legendary chestplate belonged to the Jad \
Knight.", 20)

Dragon_Chest2 = Chest("Dragon's Improved Chest", "This improved legendary chestplate \
belonged to the Jad Knight. You found it!", 21)
#-------------------------------------------------------------------------------    
...
Rune_Gauntlets = Gauntlets("Rune Gauntlets", "Crafted by the ogres of the forest. \
Only the finest and strongest ores was used in the making of these gauntlets.", 10)

Dragon_Gauntlets = Gauntlets("Dragon Gauntlets", "These gloves, crafted with the fire \
crystals of the mountain, were worn by the Jad Knight himself.", 20)

Dragon_Gauntlets2 = Gauntlets("Dragon Improved Gauntlets", "These improved gloves, \
crafted with the fire crystals of the mountain, were worn by the Jad Knight himself.", 21)
#-------------------------------------------------------------------------------
class Boots(Armour):
    def __init__(self, name, description, defense):
...
Rune_Boots = Boots("Rune Boots", "Crafted by the ogres of the forest. Only \
the finest and strongest ores was used in the making of these boots.", 10) 

Dragon_Boots = Boots("Dragon Boots", "Crafted with the sturdiest fire \
crystals of the mountain.", 20)

Dragon_Boots2 = Boots("Dragon Improved Boots", " Crafted with the sturdiest \
...
#-------------------------------------------------------------------------------        
#The enemy xp is added to player xp so the player levels up
...
#Turn based combat
    def turn_based(self, enemy):
        ...
        if enemy.health > 0:
            ...
            e_boots = enemy.boots
            #Prints the player and enemy stats each turn    
            print
            ...
            while enemy.health > 0:
                ...
                if "1" in command:
                    ...
                    if random_n == 3:
                        ...
                        Damage_Taken.number += round(e_damage)
                    #Prints stats after the first turn of combat    
                    print ("----------------------------------------------")
                    enemy.e_balance_health()
                    ...
                    print ("----------------------------------------------")  
                #If the play does not have enough magic 
...
Ogre = Enemy("Ogre", 1, 10, "Normally bashful, they will act exceedingly \
...
Sorcerer1 = Enemy("First Sorcerer", 1, 10, "A human possesing magical attributes. ",
105.0, 105.0, 45.0, 90.0, 80.0, Empty_Physical, Emerald_Skull, Empty_Helmet, Empty_Chest, Empty_Gauntlets, Empty_Boots)

Sorcerer2 = Enemy("Second Sorcerer", 1, 10, "A human possesing magical attributes. ",
105.0, 105.0, 45.0, 90.0, 80.0, Empty_Physical, Emerald_Skull, Empty_Helmet, Empty_Chest, Empty_Gauntlets, Empty_Boots)
###################################################################################################################################################
#CHAPTER TWO ENEMIES
...
awoken by the populace of Jad Town. As a result, they now leave their homes to \
...
by the populace of Jad Town. As a result, they now leave their homes to hunt those \
...
Knight1 = Enemy("Eldrith the Jad Knight", 4, 20, "A knight belonging to the highest \
rank of the Knights of Jad. What is he doing here?", 
150.0, 150.0, 50.0, 100.0, 125.0, Dragon_Scimitar, Empty_Magical, Empty_Helmet, Empty_Chest, Empty_Gauntlets, Empty_Boots)

Knight2 = Enemy("Almerth the Jad Knight", 4, 20, "A knight belonging to the highest \
rank of the Knights of Jad. What is he doing here?", 
150.0, 150.0, 50.0, 100.0, 125.0, Dragon_Scimitar, Empty_Magical, Empty_Helmet, Empty_Chest, Empty_Gauntlets, Dragon_Boots)

Knight3 = Enemy("Kanthos the Jad Knight", 4, 20, "A knight belonging to the highest \
rank of the Knights of Jad. What is he doing here?", 
150.0, 150.0, 60.0, 105.0, 125.0, Dragon_Scimitar, Empty_Magical, Empty_Helmet, Empty_Chest, Dragon_Gauntlets, Empty_Boots)

Knight4 = Enemy("Resmoth the Jad Knight", 4, 20, "A knight belonging to the highest \
rank of the Knights of Jad. What is he doing here?", 
160.0, 160.0, 60.0, 105.0, 125.0, Dragon_Scimitar, Empty_Magical, Empty_Helmet, Dragon_Chest, Empty_Gauntlets, Empty_Boots)
...
Jad = Enemy("Jad", 5, 50, "A man turned into a demon. His hatred for people led \
him to establishing his base in Jad Mountain and selling the world his death and \
the death of his knights. Now, alongside with the Knights of Jad, he is a human \
transformed into a monster, a demon. Hatred transforms.",
200.0, 200.0, 30.0, 135.0, 155.0, Phoenix_Sword, Ruby_Skull, Empty_Helmet, Dragon_Chest, Dragon_Gauntlets, Dragon_Boots)
#############################################################################################################################################################################
#RANDOM ENCOUNTERS IN CHAPTER 1
...
Varrock_Baron = Room("Varrock", "Varrock Baron's Manor", None, None, "Residential", None, None, None, [Flower_Pot, Book],
"You are inside the Varrock Baron's manor in the forest. You clutch the parchment \
...
Residential = Room("Varrock", "Residential Area", "Path_Junction", "Burial_Grounds", "Market", None, None, None, [Blanket],
"You are in the residential area of Varrock. The people of the village are hidden \
away in their homes, fearful for another raid on the village by the beasts of the forest.")

Market = Room("Varrock", "Marketplace", "Weaponsmith", "Fruit", "Bar", "Residential", None, None, [Chair, Spatula],
"You are in Varrock's Marketplace, where you can find anything from weapons \
to a cold drink. This place is empty, the werewolves made sure of that.")

Weaponsmith = Room("Varrock", "The Weaponsmith's", None, "Market", None, None, None, None, [Rug], 
"You are inside The Weaponsmith's, a weapons shop. Polished & metallic swords dangle \
on the wall, they are nothing but aesthetics. Bulky armour fills the stands, it would \
...
Fruit = Room("Varrock", "The Fruitstands", "Market", None, None, None, None, None, [Apple],
"Imagine a beautiful alignment of colourful fruit, positioned with immense precision. \
Each fruit is perfectly round. Each fruit is vivid and radiating with alluring \
colouration. Now, throw all of that away, and imagine a pathetic assortment of rotten \
crop. Actually, there is no need to imagine the revolting imagery, %s, you are looking \
right at it. " %name)

Bar = Room("Varrock", "Mar's Bar", None, None, None, "Market", None, None, [Shotglass],
"You are inside the Mar's Bar, the village's most popular social center. Come back \
later for a free drink, eh?")

Burial_Grounds = Room("Varrock", "Burial Grounds", "Residential", None, None, None, None, None, [Gnome],
...
Path_Junction = Room("Varrock", "Path to the Junction", "Junction", "Residential", None, None, None, None, [Empty],
"You are on the path to the Junction, where four paths meet.")

Path_Junction2 = Room("Grey Forest", "Path to Varrock", "Junction", "Residential", None, None, None, None, [Empty],
"You are on the path to Varrock.")

Junction = Room("Grey Forest", "Junction", "Road_Jad", "Path_Junction2", "Path_Lake", "Path_Wolf", None, None, [Empty],
...
to Varrock. The road to the north will lead you to Jad Mountain.")
...
fauna. Colourful flowers and rotten corpses situate themselves on the forest's floor.")

Flower_Bed = Room("Grey Forest", "Flower Bed", None, "North_Lake", None, None, None, None, [Rune_Boots],
"You found the ogre's shrine, a luscious flower bed of various plants and colourful \
flowers. A shrine commemorated by the townspeople of Varrock to honor the ogre's \
efforts to keep the forest \"safe\". You spot a glassy and viridescent chest. Based \
on the markings and the colour of the chest, it belongs to the ogre of the forest.")
...
on top a stalagmite. Deep scratch marks fill the walls. Taupe coloured hair lies on \
...
Hidden_Room_Lake = Room("Grey Forest", "Hidden Room", None, "Cave_East_Lake", None, None, None, None, [Rune_Gauntlets],
...
belonging to a ogre based on the markings and the colour.")
...
"You are inside what appears to be a hut that once belonged to a citizen on Varrock. \
...
decapitated ogre lies dead on the floor. There is a trail of blood, monster blood, \
leading downstairs.")

Basement_Hut = Room("Grey Forest", "Basement", None, None, None, None, "Hut", None, [Rune_Chest],
"You are in the hut's basement. It is rather dark down here. The ogre's head lies on \
the basement's floor. There is a rune chest, the ogre must have died protecting \
...
Your defense is really low, why don't you go explore? *You need better armour*")
...
magic nearby, it is emitting from the olive coloured emerald skull resting on top \
...
"You are in a pile of human and ogre corpses. The stench of tensomes of rotting \
...
Wolf_Den = Room("Grey Forest", "The Wolf's Den", None, "Cave_Entrance", None, None, None, None, [Wolf_Mask],
...
Road_Jad = Room("Grey Forest / Jad Mountain", "Road to Jad Mountain", None, "Junction", None, None, None, None, [Empty],
"You are on the road to Jad Mountian, a domain dominated by volcanic activity. You \
can not leave Grey Forest until you have completed your quest.")

Road_Jad2 = Room("Grey Forest / Jad Mountain", "Road to Jad Mountain", None, "Junction", None, None, None, None, [Empty],
...
Gate_Jad = Room("Jad Mountain", "The Gate", "Town_Square", "Road_Jad2", None, None, None, None, [Empty],
"After hours of walking down the path, you finally reach the gates of Jad Mountain, \
a domain named after the Jad Knight, one of the fiercest warriors in existence. \
Go through the gates and speak with the Jad Chief, he will have a quest for you.")

Town_Square = Room("Jad Town", "Town Square", "Jad_Stairway", None, "Chief_Palace", "Jad_Market", None, None, [Empty],
"You are in Jad Town's, very own, town square. This place used to be a striving \
market for ores and minerals. Now, it is a ravaged and desolate place. The marketplace \
and the Jad Chief's palace are nearby.")

Jad_Market = Room("Jad Town", "Marketplace", "Sword_Shop", "Jad_Fruit", "Town_Square", "Jad_Bar", None, None, [Empty],
"You are in Jad Town's marketplace. The area is on fire, the buildings have been \
...
Sword_Shop = Room("Jad Town", "The Sword Shop", None, "Jad_Market", None, None, None, None, [Empty],
...
Jad_Fruit = Room("Jad Town", "The Fruitstands", "Town_Square", None, None, None, None, None, [Empty],
"You are at the fruitstands, where the people of Jad Town came to buy their fresh \
product. The ground is splattered with fruit of all colours, it is a great contrast \
compared to the gray and mundane sky.")

Jad_Bar = Room("Jad Town", "The Jad Bar", None, None, "Jad_Market", None, None, None, [Empty],
"You are inside The Jad Bar. You won't be receiving any free drinks this time. \
There is no one to serve you, everyone is dead.")

Chief_Palace = Room("Jad Town", "Jad Chief's Palace", None, None, None, "Town_Square", None, None, [PhoenixQ],
"You are inside the Jad Chief's Palace. It is a colossal building with decorum \
appropiate for a wealthy leader like the Jad Chief. The place looks intact, as if \
the invasion of Jad Town never occured. The remaining population of the town is \
in here, they are afraid. Guards are positioned in every corner, they await the \
looming attack from Jad Mountain. The Jad Chief points at the coffee coloured oak \
table, there is a parchment atop. It is your next quest, pick it up and read it.")

Jad_Stairway = Room("Jad Mountain", "Stairway to the Mountain's Interior", None , "Town_Square", None, None, None, None, [Empty],
"You are in the stone stairway leading to the dungeon hidden away the inside of \
the mountain. You spot the Burning Tree at the top of the mountain, or Jad Summit. \
An inferno awaits. ")

Fog_Room = Room("Jad Mountain (First Floor)", "Foggy Room", None, None, "Droppings", "Skulls_Bones", None, None, [Empty],
"You are inside the first room of Jad Mountain, many more await. Your surroundings \
are obscured by a thick layer of fog. There is a door ahead, it appears to be locked.")

Droppings = Room("Jad Mountain (First Floor)", "Droppings", None, None, None, "Fog_Room", None, None, [Key1],
...
Skulls_Bones = Room("Jad Mountain (First Floor)", "Skulls & Bones",  None, None, "Fog_Room", None, None, None, [Empty],
...
Thousand_Steps = Room("Jad Mountain (First Floor)", "Thousand Steps", None, None, None, None, "Dark_Room", None, [Empty],
...
Dark_Room = Room("Jad Mountain (Second Floor)", "Lights Out", None, None, "Illuminated_Passage", None, None, None, [Empty],
...
Illuminated_Passage = Room("Jad Mountain (Second Floor)", "Illuminated Passage", "Bone_Piles", None, "Lava_LakeX", "Dark_Room", None, None, [Empty],
...
Bone_Piles = Room("Jad Mountain (Second Floor)", "Pile of Bones", None, "Illuminated_Passage", "Jad_Chest1", None, None, None, [Empty],
...
Jad_Chest1 = Room("Jad Mountain (Second Floor)", "The Room With The Boots", None, None, None, "Bone_Piles", None, None, [Dragon_Boots],
...
Lava_LakeX = Room("Jad Mountain (Second Floor)", "Lava Lake", "Stairwell", None, None, "Illuminated_Passage", None, None, [Empty],
...
Lava_Lake1 = Room("Jad Mountain (Second Floor)", "Lava Lake", "Stairwell", None, None, "Illuminated_Passage", None, None, [Empty],
"The boots allow you to walk on lava! This reminds you of a certain someone who \
could do something similar... Who was it again?")

Stairwell = Room("Jad Mountain (Second Floor)", "Stairwell", None, "Lava_LakeX", None, None, "The_Torch", None, [Empty],
...
The_Torch = Room("Jad Mountain (Third Floor)",  "The Torch", "Puddles", None, None, None, None, None, [Empty],
...
Puddles = Room("Jad Mountain (Third Floor)", "Puddles", None, "The_Torch", "Guattor", "Pedestal", None, None, [Empty],
...
Pedestal = Room("Jad Mountain (Third Floor)", "Sword on the Pedestal", None, None, "Puddles", None, None, None, [Obsidian_Sword],
...
Guattor = Room("Jad Mountain (Third Floor)", "Et Guattor Iudicia", "Primus", None, None, "Puddles", None, None, [Empty],
"You are inside the first of the Challenge Nodes, a set of challenges designed to \
kill off the weak. Four monsters await you.")

Primus = Room("Jad Mountain (Third Floor)", "In Primo Iudicii", None, None, "Secundo", None, None, None, [Empty],
...
Secundo = Room("Jad Mountain (Third Floor)", "In Secundo Iudicio", None, "Tertium", None, "Primus", None, None, [Empty],
...
Tertium = Room("Jad Mountain (Third Floor)", "Tertium Temptationis", "Secundo", None, "Quartam", None, None, None, [Empty],
...
Quartam = Room("Jad Mountain (Third Floor)", "Quartam Causam", "Reward", None, None, "Tertium", None, None, [Empty],
...
Reward = Room("Jad Mountain (Third Floor)", "Praemium et Curare", None, None, "TStairs", None, None, None, [Dragon_Chest],
...
TStairs = Room("Jad Mountain (Third Floor)", "The Stairs Leading Upwards", None, None, None, "Reward", "Three_Crystals", None, [Empty],
"The stairs will take you to the fourth floor of Jad Mountain. You did a great job \
back there, it was not easy taking down those four demons. The Knights of Jad sure \
put up a good fight...")

Three_Crystals = Room("Jad Mountain (Fourth Floor)", "Trois Cristaux", "PuzzleA", "BPhoenix", "PuzzleB", "PuzzleC", None, None, [Empty],
...
PuzzleA = Room("Jad Mountain (Fourth Floor)", "Puzzle A", None, "Three_Crystals", None, None, None, None, [Empty],
...
PuzzleB = Room("Jad Mountain (Fourth Floor)", "Puzzle B", None, None, None, "Three_Crystals", None, None, [Empty],
"You are inside a room with one glowing, green crystal hanging from the roof. There \
is also a wooden chair and table with a piece of parchment, quill, and ink.")

PuzzleC = Room("Jad Mountain (Fourth Floor)", "Puzzle C", None, None, "Three_Crystals", None, None, None, [Empty],
"You are inside a room with one glowing, red crystal hanging from the roof. There \
is also a wooden chair and table with a piece of parchment, quill, and ink.")

BPhoenix = Room("Jad Mountain (Fourth Floor)", "Nigrum Avem's Cell", "Three_Crystals", None, None, None, None, None, [Phoenix_Mask, Dragon_Gauntlets, Phoenix_Sword, Ruby_Skull],
...
deduce that this treasure room belongs to the Knights of Jad based on the ancient \
...
To_Summit = Room("Jad Mountain (Fourth Floor)", "Stairs to Jad Summit", None, "Three_Crystals", None, None, "Foot_Prints", None, [Empty],
"You are standing on the phantom stairs. The staircase leads to the summit of Jad \
Mountain, also known as Jad Summit, the most dangerous location of Jad Mountain.")

Foot_Prints = Room("Jad Summit", "Bellum", None, "Burning_Tree", None, None, None, None, [Empty],
"You went up the stairs and exited the dungeon within Jad Mountain. The boss should \
...
Burning_Tree = Room("Jad Summit", "Tree on Fire", "Foot_Prints", None, "Dark_Figure", "Vanished", None, None, [Empty],
...
mountain and spot Jad Town, you can also faintly see the Grey Forest from here. \
eTh aMn hWo Borke Teh ouFrht Wlla si eanr.")

Dark_Figure = Room("Jad Summit", "The Man of the Shadows", None, None, None, "Burning_Tree", None, None, [Empty], 
...
Altar = Room("Jad Summit", "Sacrificial Altar", "Kill", "Vanished", None, None, None, None, [Empty],
...
Kill = Room("Jad Summit", "Finis", None, None, None, None, None, "Jad_Stairway", [Empty],
"You are alongside the corpse of Jad, leader of the Knights of Jad and a warrior \
...
#Save File Number 1
def save1():
    global File1_Name, name, player, node, Movement_Counter, Enemy_Counter, Inventory_Counter, Stats_Counter, Help_Counter, Map_Counter, Misc_Counter, Damage_Dealt, MDamage_Dealt, Damage_Taken, MDamage_Taken, Bar_Counter, Candle_Counter, APuzzle, BPuzzle, CPuzzle, Werewolf1, Ogre, Werewolf2, Werewolf3, Werewolf4, Sorcerer1, Sorcerer2, Phoenix1, Phoenix2, Bat1, Bat2, Bat3, Knight1, Knight2, Knight3, Knight4, Avis, Jad, WerewolfR1, WerewolfR2, WerewolfR3, WerewolfR4, Varrock_Baron, Residential, Market, Weaponsmith, Fruit, Bar, Burial_Grounds, Path_Junction, Path_Junction2, Junction, Path_Lake, Path_Lake2, West_Lake, North_Lake, Flower_Bed, East_Lake, Cave_East_Lake, Hidden_Room_Lake, South_Lake, Hut, Basement_Hut, Lake_Azure, Path_Wolf, Path_Wolf2, Wolf_Totem, Necromancer_Cabin, N_Upstairs, N_Basement, Bone_Pile, Cave_Entrance, Wolf_Den, Road_Jad, Road_Jad2, Gate_Jad, Town_Square, Jad_Market, Sword_Shop, Jad_Fruit, Jad_Bar, Chief_Palace, Jad_Stairway, Fog_Room, Droppings, Skulls_Bones, Thousand_Steps, Dark_Room, Illuminated_Passage, Bone_Piles, Jad_Chest1, Lava_LakeX, Lava_Lake1, Stairwell, The_Torch, Puddles, Pedestal, Guattor, Primus, Secundo, Tertium, Quartam, Reward, TStairs, Three_Crystals, PuzzleA, PuzzleB, PuzzleC, BPhoenix, To_Summit, Foot_Prints, Burning_Tree, Dark_Figure, Vanished, Altar, Kill
    File1_Name = node.name
    with open('savegame1.dat', 'wb') as Save_File1:
       pickle.dump([File1_Name, name, player, node, Movement_Counter, Enemy_Counter, Inventory_Counter, Stats_Counter, Help_Counter, Map_Counter, Misc_Counter, Damage_Dealt, MDamage_Dealt, Damage_Taken, MDamage_Taken, Bar_Counter, Candle_Counter, APuzzle, BPuzzle, CPuzzle, Werewolf1, Ogre, Werewolf2, Werewolf3, Werewolf4, Sorcerer1, Sorcerer2, Phoenix1, Phoenix2, Bat1, Bat2, Bat3, Knight1, Knight2, Knight3, Knight4, Avis, Jad, WerewolfR1, WerewolfR2, WerewolfR3, WerewolfR4, Varrock_Baron, Residential, Market, Weaponsmith, Fruit, Bar, Burial_Grounds, Path_Junction, Path_Junction2, Junction, Path_Lake, Path_Lake2, West_Lake, North_Lake, Flower_Bed, East_Lake, Cave_East_Lake, Hidden_Room_Lake, South_Lake, Hut, Basement_Hut, Lake_Azure, Path_Wolf, Path_Wolf2, Wolf_Totem, Necromancer_Cabin, N_Upstairs, N_Basement, Bone_Pile, Cave_Entrance, Wolf_Den, Road_Jad, Road_Jad2, Gate_Jad, Town_Square, Jad_Market, Sword_Shop, Jad_Fruit, Jad_Bar, Chief_Palace, Jad_Stairway, Fog_Room, Droppings, Skulls_Bones, Thousand_Steps, Dark_Room, Illuminated_Passage, Bone_Piles, Jad_Chest1, Lava_LakeX, Lava_Lake1, Stairwell, The_Torch, Puddles, Pedestal, Guattor, Primus, Secundo, Tertium, Quartam, Reward, TStairs, Three_Crystals, PuzzleA, PuzzleB, PuzzleC, BPhoenix, To_Summit, Foot_Prints, Burning_Tree, Dark_Figure, Vanished, Altar, Kill], Save_File1, protocol = 2) #variables that you want to save should be here 
    print
    print ("------------------------------------------------------------------------")
    print
    print ("/// Save Files %s" %("\\\\\\"))
...
#Save File Number 2
def save2():
    global File2_Name, name, player, node, Movement_Counter, Enemy_Counter, Inventory_Counter, Stats_Counter, Help_Counter, Map_Counter, Misc_Counter, Damage_Dealt, MDamage_Dealt, Damage_Taken, MDamage_Taken, Bar_Counter, Candle_Counter, APuzzle, BPuzzle, CPuzzle, Werewolf1, Ogre, Werewolf2, Werewolf3, Werewolf4, Sorcerer1, Sorcerer2, Phoenix1, Phoenix2, Bat1, Bat2, Bat3, Knight1, Knight2, Knight3, Knight4, Avis, Jad, WerewolfR1, WerewolfR2, WerewolfR3, WerewolfR4, Varrock_Baron, Residential, Market, Weaponsmith, Fruit, Bar, Burial_Grounds, Path_Junction, Path_Junction2, Junction, Path_Lake, Path_Lake2, West_Lake, North_Lake, Flower_Bed, East_Lake, Cave_East_Lake, Hidden_Room_Lake, South_Lake, Hut, Basement_Hut, Lake_Azure, Path_Wolf, Path_Wolf2, Wolf_Totem, Necromancer_Cabin, N_Upstairs, N_Basement, Bone_Pile, Cave_Entrance, Wolf_Den, Road_Jad, Road_Jad2, Gate_Jad, Town_Square, Jad_Market, Sword_Shop, Jad_Fruit, Jad_Bar, Chief_Palace, Jad_Stairway, Fog_Room, Droppings, Skulls_Bones, Thousand_Steps, Dark_Room, Illuminated_Passage, Bone_Piles, Jad_Chest1, Lava_LakeX, Lava_Lake1, Stairwell, The_Torch, Puddles, Pedestal, Guattor, Primus, Secundo, Tertium, Quartam, Reward, TStairs, Three_Crystals, PuzzleA, PuzzleB, PuzzleC, BPhoenix, To_Summit, Foot_Prints, Burning_Tree, Dark_Figure, Vanished, Altar, Kill
    File2_Name = node.name
    with open('savegame2.dat', 'wb') as Save_File2:
       pickle.dump([File2_Name, name, player, node, Movement_Counter, Enemy_Counter, Inventory_Counter, Stats_Counter, Help_Counter, Map_Counter, Misc_Counter, Damage_Dealt, MDamage_Dealt, Damage_Taken, MDamage_Taken, Bar_Counter, Candle_Counter, APuzzle, BPuzzle, CPuzzle, Werewolf1, Ogre, Werewolf2, Werewolf3, Werewolf4, Sorcerer1, Sorcerer2, Phoenix1, Phoenix2, Bat1, Bat2, Bat3, Knight1, Knight2, Knight3, Knight4, Avis, Jad, WerewolfR1, WerewolfR2, WerewolfR3, WerewolfR4, Varrock_Baron, Residential, Market, Weaponsmith, Fruit, Bar, Burial_Grounds, Path_Junction, Path_Junction2, Junction, Path_Lake, Path_Lake2, West_Lake, North_Lake, Flower_Bed, East_Lake, Cave_East_Lake, Hidden_Room_Lake, South_Lake, Hut, Basement_Hut, Lake_Azure, Path_Wolf, Path_Wolf2, Wolf_Totem, Necromancer_Cabin, N_Upstairs, N_Basement, Bone_Pile, Cave_Entrance, Wolf_Den, Road_Jad, Road_Jad2, Gate_Jad, Town_Square, Jad_Market, Sword_Shop, Jad_Fruit, Jad_Bar, Chief_Palace, Jad_Stairway, Fog_Room, Droppings, Skulls_Bones, Thousand_Steps, Dark_Room, Illuminated_Passage, Bone_Piles, Jad_Chest1, Lava_LakeX, Lava_Lake1, Stairwell, The_Torch, Puddles, Pedestal, Guattor, Primus, Secundo, Tertium, Quartam, Reward, TStairs, Three_Crystals, PuzzleA, PuzzleB, PuzzleC, BPhoenix, To_Summit, Foot_Prints, Burning_Tree, Dark_Figure, Vanished, Altar, Kill], Save_File2, protocol = 2) #variables that you want to save should be here 
    print
    print ("------------------------------------------------------------------------")
    print
    print ("/// Save Files %s" %("\\\\\\"))
...
#Save File Number 3
def save3():
    global File3_Name, name, player, node, Movement_Counter, Enemy_Counter, Inventory_Counter, Stats_Counter, Help_Counter, Map_Counter, Misc_Counter, Damage_Dealt, MDamage_Dealt, Damage_Taken, MDamage_Taken, Bar_Counter, Candle_Counter, APuzzle, BPuzzle, CPuzzle, Werewolf1, Ogre, Werewolf2, Werewolf3, Werewolf4, Sorcerer1, Sorcerer2, Phoenix1, Phoenix2, Bat1, Bat2, Bat3, Knight1, Knight2, Knight3, Knight4, Avis, Jad, WerewolfR1, WerewolfR2, WerewolfR3, WerewolfR4, Varrock_Baron, Residential, Market, Weaponsmith, Fruit, Bar, Burial_Grounds, Path_Junction, Path_Junction2, Junction, Path_Lake, Path_Lake2, West_Lake, North_Lake, Flower_Bed, East_Lake, Cave_East_Lake, Hidden_Room_Lake, South_Lake, Hut, Basement_Hut, Lake_Azure, Path_Wolf, Path_Wolf2, Wolf_Totem, Necromancer_Cabin, N_Upstairs, N_Basement, Bone_Pile, Cave_Entrance, Wolf_Den, Road_Jad, Road_Jad2, Gate_Jad, Town_Square, Jad_Market, Sword_Shop, Jad_Fruit, Jad_Bar, Chief_Palace, Jad_Stairway, Fog_Room, Droppings, Skulls_Bones, Thousand_Steps, Dark_Room, Illuminated_Passage, Bone_Piles, Jad_Chest1, Lava_LakeX, Lava_Lake1, Stairwell, The_Torch, Puddles, Pedestal, Guattor, Primus, Secundo, Tertium, Quartam, Reward, TStairs, Three_Crystals, PuzzleA, PuzzleB, PuzzleC, BPhoenix, To_Summit, Foot_Prints, Burning_Tree, Dark_Figure, Vanished, Altar, Kill
    File3_Name = node.name
    with open('savegame3.dat', 'wb') as Save_File3:
       pickle.dump([File3_Name, name, player, node, Movement_Counter, Enemy_Counter, Inventory_Counter, Stats_Counter, Help_Counter, Map_Counter, Misc_Counter, Damage_Dealt, MDamage_Dealt, Damage_Taken, MDamage_Taken, Bar_Counter, Candle_Counter, APuzzle, BPuzzle, CPuzzle, Werewolf1, Ogre, Werewolf2, Werewolf3, Werewolf4, Sorcerer1, Sorcerer2, Phoenix1, Phoenix2, Bat1, Bat2, Bat3, Knight1, Knight2, Knight3, Knight4, Avis, Jad, WerewolfR1, WerewolfR2, WerewolfR3, WerewolfR4, Varrock_Baron, Residential, Market, Weaponsmith, Fruit, Bar, Burial_Grounds, Path_Junction, Path_Junction2, Junction, Path_Lake, Path_Lake2, West_Lake, North_Lake, Flower_Bed, East_Lake, Cave_East_Lake, Hidden_Room_Lake, South_Lake, Hut, Basement_Hut, Lake_Azure, Path_Wolf, Path_Wolf2, Wolf_Totem, Necromancer_Cabin, N_Upstairs, N_Basement, Bone_Pile, Cave_Entrance, Wolf_Den, Road_Jad, Road_Jad2, Gate_Jad, Town_Square, Jad_Market, Sword_Shop, Jad_Fruit, Jad_Bar, Chief_Palace, Jad_Stairway, Fog_Room, Droppings, Skulls_Bones, Thousand_Steps, Dark_Room, Illuminated_Passage, Bone_Piles, Jad_Chest1, Lava_LakeX, Lava_Lake1, Stairwell, The_Torch, Puddles, Pedestal, Guattor, Primus, Secundo, Tertium, Quartam, Reward, TStairs, Three_Crystals, PuzzleA, PuzzleB, PuzzleC, BPhoenix, To_Summit, Foot_Prints, Burning_Tree, Dark_Figure, Vanished, Altar, Kill], Save_File3, protocol = 2) #variables that you want to save should be here 
    print 
    print ("------------------------------------------------------------------------")
    print
    print ("/// Save Files %s" %("\\\\\\"))
...
#Load File Number 1
def load1():
    global Load1_Name, name, player, node, Movement_Counter, Enemy_Counter, Inventory_Counter, Stats_Counter, Help_Counter, Map_Counter, Misc_Counter, Damage_Dealt, MDamage_Dealt, Damage_Taken, MDamage_Taken, Bar_Counter, Candle_Counter, APuzzle, BPuzzle, CPuzzle, Werewolf1, Ogre, Werewolf2, Werewolf3, Werewolf4, Sorcerer1, Sorcerer2, Phoenix1, Phoenix2, Bat1, Bat2, Bat3, Knight1, Knight2, Knight3, Knight4, Avis, Jad, WerewolfR1, WerewolfR2, WerewolfR3, WerewolfR4, Varrock_Baron, Residential, Market, Weaponsmith, Fruit, Bar, Burial_Grounds, Path_Junction, Path_Junction2, Junction, Path_Lake, Path_Lake2, West_Lake, North_Lake, Flower_Bed, East_Lake, Cave_East_Lake, Hidden_Room_Lake, South_Lake, Hut, Basement_Hut, Lake_Azure, Path_Wolf, Path_Wolf2, Wolf_Totem, Necromancer_Cabin, N_Upstairs, N_Basement, Bone_Pile, Cave_Entrance, Wolf_Den, Road_Jad, Road_Jad2, Gate_Jad, Town_Square, Jad_Market, Sword_Shop, Jad_Fruit, Jad_Bar, Chief_Palace, Jad_Stairway, Fog_Room, Droppings, Skulls_Bones, Thousand_Steps, Dark_Room, Illuminated_Passage, Bone_Piles, Jad_Chest1, Lava_LakeX, Lava_Lake1, Stairwell, The_Torch, Puddles, Pedestal, Guattor, Primus, Secundo, Tertium, Quartam, Reward, TStairs, Three_Crystals, PuzzleA, PuzzleB, PuzzleC, BPhoenix, To_Summit, Foot_Prints, Burning_Tree, Dark_Figure, Vanished, Altar, Kill
    Load1_Name = File1_Name
    with open('savegame1.dat', 'rb') as Load_File1:
        Load1_Name, name, player, node, Movement_Counter, Enemy_Counter, Inventory_Counter, Stats_Counter, Help_Counter, Map_Counter, Misc_Counter, Damage_Dealt, MDamage_Dealt, Damage_Taken, MDamage_Taken, Bar_Counter, Candle_Counter, APuzzle, BPuzzle, CPuzzle, Werewolf1, Ogre, Werewolf2, Werewolf3, Werewolf4, Sorcerer1, Sorcerer2, Phoenix1, Phoenix2, Bat1, Bat2, Bat3, Knight1, Knight2, Knight3, Knight4, Avis, Jad, WerewolfR1, WerewolfR2, WerewolfR3, WerewolfR4, Varrock_Baron, Residential, Market, Weaponsmith, Fruit, Bar, Burial_Grounds, Path_Junction, Path_Junction2, Junction, Path_Lake, Path_Lake2, West_Lake, North_Lake, Flower_Bed, East_Lake, Cave_East_Lake, Hidden_Room_Lake, South_Lake, Hut, Basement_Hut, Lake_Azure, Path_Wolf, Path_Wolf2, Wolf_Totem, Necromancer_Cabin, N_Upstairs, N_Basement, Bone_Pile, Cave_Entrance, Wolf_Den, Road_Jad, Road_Jad2, Gate_Jad, Town_Square, Jad_Market, Sword_Shop, Jad_Fruit, Jad_Bar, Chief_Palace, Jad_Stairway, Fog_Room, Droppings, Skulls_Bones, Thousand_Steps, Dark_Room, Illuminated_Passage, Bone_Piles, Jad_Chest1, Lava_LakeX, Lava_Lake1, Stairwell, The_Torch, Puddles, Pedestal, Guattor, Primus, Secundo, Tertium, Quartam, Reward, TStairs, Three_Crystals, PuzzleA, PuzzleB, PuzzleC, BPhoenix, To_Summit, Foot_Prints, Burning_Tree, Dark_Figure, Vanished, Altar, Kill = pickle.load(Load_File1)
    print
    print ("Game succesfully loaded!")
#-------------------------------------------------------------------------------    
#Load File Number 2
def load2():
    global Load2_Name, name, player, node, Movement_Counter, Enemy_Counter, Inventory_Counter, Stats_Counter, Help_Counter, Map_Counter, Misc_Counter, Damage_Dealt, MDamage_Dealt, Damage_Taken, MDamage_Taken, Bar_Counter, Candle_Counter, APuzzle, BPuzzle, CPuzzle, Werewolf1, Ogre, Werewolf2, Werewolf3, Werewolf4, Sorcerer1, Sorcerer2, Phoenix1, Phoenix2, Bat1, Bat2, Bat3, Knight1, Knight2, Knight3, Knight4, Avis, Jad, WerewolfR1, WerewolfR2, WerewolfR3, WerewolfR4, Varrock_Baron, Residential, Market, Weaponsmith, Fruit, Bar, Burial_Grounds, Path_Junction, Path_Junction2, Junction, Path_Lake, Path_Lake2, West_Lake, North_Lake, Flower_Bed, East_Lake, Cave_East_Lake, Hidden_Room_Lake, South_Lake, Hut, Basement_Hut, Lake_Azure, Path_Wolf, Path_Wolf2, Wolf_Totem, Necromancer_Cabin, N_Upstairs, N_Basement, Bone_Pile, Cave_Entrance, Wolf_Den, Road_Jad, Road_Jad2, Gate_Jad, Town_Square, Jad_Market, Sword_Shop, Jad_Fruit, Jad_Bar, Chief_Palace, Jad_Stairway, Fog_Room, Droppings, Skulls_Bones, Thousand_Steps, Dark_Room, Illuminated_Passage, Bone_Piles, Jad_Chest1, Lava_LakeX, Lava_Lake1, Stairwell, The_Torch, Puddles, Pedestal, Guattor, Primus, Secundo, Tertium, Quartam, Reward, TStairs, Three_Crystals, PuzzleA, PuzzleB, PuzzleC, BPhoenix, To_Summit, Foot_Prints, Burning_Tree, Dark_Figure, Vanished, Altar, Kill
    Load2_Name = File2_Name
    with open('savegame2.dat', 'rb') as Load_File2:
        Load2_Name, name, player, node, Movement_Counter, Enemy_Counter, Inventory_Counter, Stats_Counter, Help_Counter, Map_Counter, Misc_Counter, Damage_Dealt, MDamage_Dealt, Damage_Taken, MDamage_Taken, Bar_Counter, Candle_Counter, APuzzle, BPuzzle, CPuzzle, Werewolf1, Ogre, Werewolf2, Werewolf3, Werewolf4, Sorcerer1, Sorcerer2, Phoenix1, Phoenix2, Bat1, Bat2, Bat3, Knight1, Knight2, Knight3, Knight4, Avis, Jad, WerewolfR1, WerewolfR2, WerewolfR3, WerewolfR4, Varrock_Baron, Residential, Market, Weaponsmith, Fruit, Bar, Burial_Grounds, Path_Junction, Path_Junction2, Junction, Path_Lake, Path_Lake2, West_Lake, North_Lake, Flower_Bed, East_Lake, Cave_East_Lake, Hidden_Room_Lake, South_Lake, Hut, Basement_Hut, Lake_Azure, Path_Wolf, Path_Wolf2, Wolf_Totem, Necromancer_Cabin, N_Upstairs, N_Basement, Bone_Pile, Cave_Entrance, Wolf_Den, Road_Jad, Road_Jad2, Gate_Jad, Town_Square, Jad_Market, Sword_Shop, Jad_Fruit, Jad_Bar, Chief_Palace, Jad_Stairway, Fog_Room, Droppings, Skulls_Bones, Thousand_Steps, Dark_Room, Illuminated_Passage, Bone_Piles, Jad_Chest1, Lava_LakeX, Lava_Lake1, Stairwell, The_Torch, Puddles, Pedestal, Guattor, Primus, Secundo, Tertium, Quartam, Reward, TStairs, Three_Crystals, PuzzleA, PuzzleB, PuzzleC, BPhoenix, To_Summit, Foot_Prints, Burning_Tree, Dark_Figure, Vanished, Altar, Kill = pickle.load(Load_File2)
    print
    print ("Game succesfully loaded!")
#-------------------------------------------------------------------------------    
#Load File Number 3
def load3():
    global Load3_Name, name, player, node, Movement_Counter, Enemy_Counter, Inventory_Counter, Stats_Counter, Help_Counter, Map_Counter, Misc_Counter, Damage_Dealt, MDamage_Dealt, Damage_Taken, MDamage_Taken, Bar_Counter, Candle_Counter, APuzzle, BPuzzle, CPuzzle, Werewolf1, Ogre, Werewolf2, Werewolf3, Werewolf4, Sorcerer1, Sorcerer2, Phoenix1, Phoenix2, Bat1, Bat2, Bat3, Knight1, Knight2, Knight3, Knight4, Avis, Jad, WerewolfR1, WerewolfR2, WerewolfR3, WerewolfR4, Varrock_Baron, Residential, Market, Weaponsmith, Fruit, Bar, Burial_Grounds, Path_Junction, Path_Junction2, Junction, Path_Lake, Path_Lake2, West_Lake, North_Lake, Flower_Bed, East_Lake, Cave_East_Lake, Hidden_Room_Lake, South_Lake, Hut, Basement_Hut, Lake_Azure, Path_Wolf, Path_Wolf2, Wolf_Totem, Necromancer_Cabin, N_Upstairs, N_Basement, Bone_Pile, Cave_Entrance, Wolf_Den, Road_Jad, Road_Jad2, Gate_Jad, Town_Square, Jad_Market, Sword_Shop, Jad_Fruit, Jad_Bar, Chief_Palace, Jad_Stairway, Fog_Room, Droppings, Skulls_Bones, Thousand_Steps, Dark_Room, Illuminated_Passage, Bone_Piles, Jad_Chest1, Lava_LakeX, Lava_Lake1, Stairwell, The_Torch, Puddles, Pedestal, Guattor, Primus, Secundo, Tertium, Quartam, Reward, TStairs, Three_Crystals, PuzzleA, PuzzleB, PuzzleC, BPhoenix, To_Summit, Foot_Prints, Burning_Tree, Dark_Figure, Vanished, Altar, Kill
    Load3_Name = File3_Name
    with open('savegame3.dat', 'rb') as Load_File3:
        Load3_Name, name, player, node, Movement_Counter, Enemy_Counter, Inventory_Counter, Stats_Counter, Help_Counter, Map_Counter, Misc_Counter, Damage_Dealt, MDamage_Dealt, Damage_Taken, MDamage_Taken, Bar_Counter, Candle_Counter, APuzzle, BPuzzle, CPuzzle, Werewolf1, Ogre, Werewolf2, Werewolf3, Werewolf4, Sorcerer1, Sorcerer2, Phoenix1, Phoenix2, Bat1, Bat2, Bat3, Knight1, Knight2, Knight3, Knight4, Avis, Jad, WerewolfR1, WerewolfR2, WerewolfR3, WerewolfR4, Varrock_Baron, Residential, Market, Weaponsmith, Fruit, Bar, Burial_Grounds, Path_Junction, Path_Junction2, Junction, Path_Lake, Path_Lake2, West_Lake, North_Lake, Flower_Bed, East_Lake, Cave_East_Lake, Hidden_Room_Lake, South_Lake, Hut, Basement_Hut, Lake_Azure, Path_Wolf, Path_Wolf2, Wolf_Totem, Necromancer_Cabin, N_Upstairs, N_Basement, Bone_Pile, Cave_Entrance, Wolf_Den, Road_Jad, Road_Jad2, Gate_Jad, Town_Square, Jad_Market, Sword_Shop, Jad_Fruit, Jad_Bar, Chief_Palace, Jad_Stairway, Fog_Room, Droppings, Skulls_Bones, Thousand_Steps, Dark_Room, Illuminated_Passage, Bone_Piles, Jad_Chest1, Lava_LakeX, Lava_Lake1, Stairwell, The_Torch, Puddles, Pedestal, Guattor, Primus, Secundo, Tertium, Quartam, Reward, TStairs, Three_Crystals, PuzzleA, PuzzleB, PuzzleC, BPhoenix, To_Summit, Foot_Prints, Burning_Tree, Dark_Figure, Vanished, Altar, Kill = pickle.load(Load_File3)
    print
    print ("Game succesfully loaded!")
#-------------------------------------------------------------------------------
def savefile_name():
    ...
    print ("(3) Save File: %s" %File3_Name)
#-------------------------------------------------------------------------------
#the exp needed for certain levels
def level_up():    
...
def start(): 
    ...
    while True:
        ...
        physical = ["Rune Sword", "Obsidian Sword", "Phoenix Sword"]
        magical = ["Emerald Skull", "Ruby Skull"]
        headgear = ["Legendary Werewolf Mask", "Phoenix Mask"]
        chest = ["Rune Chestplate", "Dragon Chest"]
        gauntlets = ["Rune Gauntlets", "Dragon Gauntlets"]
        boots = ["Rune Boots", "Dragon Boots"]
        other = ["Key"]
        ...
        general = ["Rune Sword", "Emerald Skull",  "Legendary Werewolf Mask", 
        "Rune Chestplate", "Rune Gauntlets", "Rune Boots", "Obsidian Sword", 
        "Phoenix Sword", "Ruby Skull", "Phoenix Mask", "Dragon Chest", "Dragon Gauntlets", 
        "Dragon Boots", "Key", "Flower Pot", "Book", "Apple", "Blanket", "Bed", "Rug", 
...
fill the walls. Taupe coloured hair lies on the cave's floor. It is apparent that \
lycanthropy took place here. The uncanny sounds are gone, you killed the werewolf."
#-------------------------------------------------------------------------------
#DESCRIPTION CHANGE       
        if player.chest == Rune_Chest:
            Basement_Hut.description = "You are in the hut's basement. It is rather \
dark down here. The ogre's head lies on the basement's floor along with the werewolf's \
corpse. The stairs will take you up to the first floor of the hut."
#-------------------------------------------------------------------------------
#DESCRIPTION CHANGE            
        if player.gauntlets == Rune_Gauntlets:
            Hidden_Room_Lake.description = "You found the cave's hidden room. It is pitch-black. \
The werewolf's corpse lies on the ground."
#-------------------------------------------------------------------------------
#DESCRIPTION CHANGE
        if player.boots == Rune_Boots:
            Flower_Bed.description = "You found the ogre's shrine, a luscious flower bed of various plants and colourful \
flowers. A shrine commemorated by the townspeople of Varrock to honor the ogre's \
efforts to keep the forest \"safe\". The sprawl of the ogre's lifeless body focuses \
the attention away from the shrine." 
#-------------------------------------------------------------------------------
#DESCRIPTION CHANGE & OPEN PATH       
        if (player.boots == Rune_Boots and player.chest == Rune_Chest and player.gauntlets == Rune_Gauntlets) and node == Path_Wolf:
...
floor. Carve its face out and wear it as a trophy for your exploit."
            ...
            Varrock_Baron.description = "You are inside the Varrock Baron's manor in the \
forest. You have completed the task of killing the leader of the wolf pack. As \
promised, you have been rewarded. Your reward is The Rune Sword."

            Residential.west = "Varrock_Baron"
#-------------------------------------------------------------------------------
#DESCRIPTION CHANGE
        if player.headgear == Wolf_Mask:
            Wolf_Den.description = "You are inside The Wolf's Den. It smells of \
blood and wet dog, a crippling stench. The Grey Wolf lies lifeless on the cave's \
floor, you wear its face as a mask. Head to the Varrock Baron's Manor to receive \
your reward."
#-------------------------------------------------------------------------------
#DESCRIPTION CHANGE, REWARD, & REMOVES QUEST
        if node == Varrock_Baron:
            if Werewolf4.health <= 0:
                if player.physical != Rune_Sword:
                    player.physical = Rune_Sword
                    print ("------------------------------------------------------------------------")
                    print ("> %s was rewarded the Rune Sword! <" %name) 
                    print ("> Open your inventory for more information <")  
                if player.physical == Rune_Sword:
                    Varrock_Baron.description = "You are inside the Varrock Baron's manor \
in the forest. You have completed the task, now it is time to leave Grey Forest \
since you have no more business here. Travel north to Jad Mountain. "
                    print 
...
#-------------------------------------------------------------------------------
#DESCRIPTION CHANGE & ENTRY TO Jad MOUNTAIN                
        if player.quest != WerewolfQ:
            Road_Jad.north = "Gate_Jad"
            Road_Jad.description = "You are on the road to Jad Mountian, a domain \
...
#NEW QUEST            
        if player.quest == PhoenixQ:
            ...
            Jad_Stairway.up = "Fog_Room"
#-------------------------------------------------------------------------------
#DESCRIPTION CHANGE & OPEN DOOR
        if player.other == Key1:
            Fog_Room.north = "Thousand_Steps"
            Fog_Room.description = "You are inside the first room of Jad Mountain, \
...
#-------------------------------------------------------------------------------
#DESCRIPTION CHANGE 
        if player.boots == Dragon_Boots:
            
            Jad_Chest1.description = "You are inside a small room."
...
at the end of the lake."
            ...
            Jad_Chest1.description = "You are inside a small room with nothing \
...
All three crystals are now glowing, each with their own respective colour. From left \
to right, their colours are: blue, red, and green. Before your very own eyes, a ghostly, \
almost transparent, and cloudy staircase is formed from nothingness. The stairs \
lead to an opening on the roof, it might be one of the exits of Jad Mountain..."
...
#QUEST (UPDATE)
        if Jad.health <= 0:
            ...
            Jad_Stairway.description = "You are in the stone stairway leading to \
the dungeon hidden away the inside of the mountain. The Burning Tree atop Jad \
Mountain has vanished..."
            Jad_Stairway.up = None
            Town_Square.east = "Chief_Palace" 
            Chief_Palace.description = "You are inside the Jad Chief's Palace. \
It is a colossal building with decorum appropiate for a wealthy leader like the \
Jad Chief. The place looks intact, as if the invasion of Jad Town never occured. \
...
positioned in every corner."
        ...
        if player.quest == PhoenixQ_Updated:
            if node == Chief_Palace:
                ...
                print ("%s" %node.description)
                print 
                print ("The Jad Chief salutes you and thanks you for completing the \
task. The town is now safe, thanks to you, and thanks to you the beasts residing \
in Jad Mountain will all be driven to extinction by the townfolk. You walk away \
...
closer and closer. You turn around and see a ogre rushing towards you. They can \
...
for the deaths of the villager and the ogre. Kill it.")
                print ("Would you like a tutorial for combat? Yes or no?")
                print
                question = input("> ")
                if question in yes:
...
pack and the fiercest wolf in the forest. Kill it and the Varrock Baron's task will be \
complete.") 
...
to be built with ogre flesh, a durable and strong material. Kill him.")
...
Suddenly, the candles in the room light up. You see an armoured humanoid rising from \
its sleep. Strangely enough, it is wearing Jad Armour. Could it be? Is that thing \
one of the four Knights of Jad?")
...
the next room and, coming to you as no surprise, a Jad Knight attempts to strike \
you. You dodge his attack and strike him. He blocks the hit with his gauntlets, he \
is wearing stronger armour relative to the other two knights. ") 
...
#COMBAT
        if node == Quartam:
            if Knight4.health > 0:
                ...
                print ("> %s, %s" %(node.name, node.domain))
                print
                print ("This is the last of the Knights of Jad that you will fight. \
You can't help but feel confused. What are the the Knights of Jad doing inside here? \
Their fates were unclear for decades, until now. They entered the mountain alongside \
Jad, hoping to kill the phoenix that ruled the mountain. They never returned, of \
...
#-------------------------------------------------------------------------------
        if node == BPhoenix:
            if Avis.health > 0:
                ...
                print ("> %s, %s" %(node.name, node.domain))
                print
                print ("The Knights of Jad's treasure room was not left alone. It \
...
#-------------------------------------------------------------------------------
        if node == Kill:
            if Jad.health > 0:
                ...
                print ("> %s, %s" %(node.name, node.domain))
                print
                print ("You are standing before Jad, leader of the Knights of Jad \
...
his voice is harsh like an ogre's.")
                print
                print ("Jad: I have gone through hell to get this far, to protect \
